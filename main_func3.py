...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 22 22:46:43 2020
...
def wrapper(threshold = 20, N = 100, groups = 10, step = 10, IC_loop = 1000):
    ...
    IC_10_best = [("indegree", [], []), ("outdegree", [], []), ("degree", [], [])]
    IC_10_random = [("indegree", [], []), ("outdegree", [], []), ("degree", [], [])]
    ...
    for nnn in range(0, N+1, step):
        ...
        #main digraph creation - random
        di_g2 = ig.Graph(directed = True)
        di_g2.add_vertices(list(g3.vs["name"]))
        
        for x in g3.es:
            if x["added"] == 0:
                di_g2.add_edge(x.tuple[0], x.tuple[1], weights = weights[x.tuple[0]][x.tuple[1]], added = x["added"])
                di_g2.add_edge(x.tuple[1], x.tuple[0], weights = weights[x.tuple[1]][x.tuple[0]], added = x["added"])
            elif x["added"] == 1:
                di_g2.add_edge(x.tuple[0], x.tuple[1], weights = 0.25, added = x["added"])
                di_g2.add_edge(x.tuple[1], x.tuple[0], weights = 0.25, added = x["added"])
        
        di_g2.vs["indegree"] = di_g2.indegree()
        di_g2.vs["outdegree"] = di_g2.outdegree()
        di_g2.vs["degree"] = di_g2.degree()
        
        
        #main digraph creation - best
        ...
        for i in range(len(metrics)):
            ...
            if nnn == 0:
                di_g_copy = deepcopy(di_g)
                di_g2_copy = deepcopy(di_g2)
                initial_grp_random.append(di_g2_copy.vs.select(lambda x:x[metrics[i]] > sorted(di_g2.vs[metrics[i]])[-16]))
                initial_grp_best.append(di_g_copy.vs.select(lambda x:x[metrics[i]] > sorted(di_g.vs[metrics[i]])[-16]))
            ...
            for j in range(IC_loop):
                di_g_copy = deepcopy(di_g)
                ic10_res_best = independent_cascades_new(di_g_copy, initial_grp_best[i]["name"])
                tmp_res_best_1.append(len(ic10_res_best[0]))
                tmp_res_best_2.append(len(ic10_res_best[1]))                           
                
                di_g2_copy = deepcopy(di_g2)
                ic10_res_random = independent_cascades_new(di_g2_copy, initial_grp_random[i]["name"])
                tmp_res_random_1.append(len(ic10_res_random[0]))
                tmp_res_random_2.append(len(ic10_res_random[1])) 
                
            IC_10_best[i][1].append(sum(tmp_res_best_1)/len(tmp_res_best_1))
            IC_10_best[i][2].append(sum(tmp_res_best_2)/len(tmp_res_best_2))
            
            IC_10_random[i][1].append(sum(tmp_res_random_1)/len(tmp_res_random_1))
            IC_10_random[i][2].append(sum(tmp_res_random_2)/len(tmp_res_random_2))
        ...
        if nnn == 0:
            ...
            ig.plot(di_g_copy, **visual_style2, mark_groups = True).save("Wyniki/{}/IC10_influenced_best_0.png".format(start_time))
            
            visual_style2["vertex_color"] = [color_dict[int(x == 1)] for x in di_g2_copy.vs["influenced"]]
            ig.plot(di_g2_copy, **visual_style2, mark_groups = True).save("Wyniki/{}/IC10_influenced_random_0.png".format(start_time))
    ...
    result.append(("Grupa inizializująca dla 10% próby (losowy graf)", initial_grp_random))
    result.append(("Grupa inizializująca dla 10% próby (najlepszy graf)", initial_grp_best))
    result.append(("wyniki modelu IC dla 10% inicjalnych węzłów (losowy)", IC_10_random))
    result.append(("wyniki modelu IC dla 10% inicjalnych węzłów (najlepszy)", IC_10_best))
    ...
    visual_style2["bbox"] = (3000, 2000)
    visual_style2["margin"] = 50
    ig.plot(di_g_copy, **visual_style2, mark_groups = True).save("Wyniki/{}/IC10_influenced_best_final.png".format(start_time))
    
    visual_style2["vertex_color"] = [color_dict[int(x == 1)] for x in di_g2_copy.vs["influenced"]]
    ig.plot(di_g2_copy, **visual_style2, mark_groups = True).save("Wyniki/{}/IC10_influenced_random_final.png".format(start_time))
    ...
    spread = pd.DataFrame(data = [list(range(0, N+1, step)) * len(IC_10_random), [y for x in IC_10_random for y in x[1]], [x for x in miary for _ in range(0, N+1, step)]], index = ["index", "spread_iter", "Miara"]).T
    ...
    sns.despine()
    fig.savefig('Wyniki/{}/IC10_random_spread_iterations.png'.format(start_time))
    
    
    #spread range dataframe
    spread2 = pd.DataFrame(data = [list(range(0, N+1, step)) * len(IC_10_random), [y for x in IC_10_random for y in x[2]], [x for x in miary for _ in range(0, N+1, step)]], index = ["index", "spread_range", "Miara"]).T
    ...
    sns.despine()
    fig.savefig('Wyniki/{}/IC10_random_spread_activations.png'.format(start_time))
    
    #IC_10_best
    #spread iterations no. dataframe
    spread3 = pd.DataFrame(data = [list(range(0, N+1, step)) * len(IC_10_best), [y for x in IC_10_best for y in x[1]], [x for x in miary for _ in range(0, N+1, step)]], index = ["index", "spread_iter", "Miara"]).T
    ...
    sns.despine()
    fig.savefig('Wyniki/{}/IC10_best_spread_iterations.png'.format(start_time))
    
    
    #spread range dataframe
    spread4 = pd.DataFrame(data = [list(range(0, N+1, step)) * len(IC_10_best), [y for x in IC_10_best for y in x[2]], [x for x in miary for _ in range(0, N+1, step)]], index = ["index", "spread_range", "Miara"]).T
    ...
    sns.despine()
    fig.savefig('Wyniki/{}/IC10_best_spread_activations.png'.format(start_time))
    
    #spread vs mod.
    spread_n = [x[0] for x in IC_10_random] 
    spread_v = [x[2] for x in IC_10_random]
    ...
    spread_n = [x[0] for x in IC_10_best] 
    spread_v = [x[2] for x in IC_10_best]
    modularity = [best_edges_mod]
    ...
    ax[0].set_title('Wykres dla losowo dodanych krawędzi', size = 20)
    ax[1].set_title('Wykres dla najlepiej wybranych krawędzi', size = 20)
